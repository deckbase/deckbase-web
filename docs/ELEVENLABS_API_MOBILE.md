...
**Server requirements for caching:** Set **`FIREBASE_STORAGE_BUCKET`** (e.g. `your-project.firebasestorage.app`) in the **hosting** env (e.g. Vercel → Project → Settings → Environment Variables), not only in `.env.prod`. The API returns 503 if this is missing. The response header **`X-Voice-Sample-Source`** is `cache`, `storage`, or `elevenlabs`—after the first request, later requests for the same `voice_id` should show `cache` or `storage`. Use the same `voice_id` from the voices list so you get the same cached sample.
