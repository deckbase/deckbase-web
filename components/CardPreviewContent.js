...
import { Eye, Check, FlipHorizontal } from "lucide-react";
...
const resolveBlockType = (type) => {
  ...
  if (typeof type === "number" && BLOCK_TYPE_BY_NUM[type])
    return BLOCK_TYPE_BY_NUM[type];
  if (typeof type === "string" && /^\d+$/.test(type))
    return BLOCK_TYPE_BY_NUM[Number(type)] || type;
  return type;
};
...
const isQuizBlock = (block) =>
  block && QUIZ_BLOCK_TYPES.has(resolveBlockType(block.type));

/** Missing / null side = front (same as study + card editor). */
const effectivePreviewSide = (block) =>
...
const getQuizData = (block, value) => {
  ...
  if (Array.isArray(config?.correctAnswers)) {
    ...
    typeof config?.correctAnswerIndex === "number" &&
    options[config.correctAnswerIndex]
  ) {
  ...
  return (
    <>
      {(blocks || []).map((block, index) => {
        ...
        const blockKey =
          block.blockId != null && block.blockId !== ""
            ? block.blockId
            : `block-${index}`;
          ...
          return (
            <div key={blockKey} className="space-y-2">
              ...
              {showAnswer && correctAnswer && (
                <p
                  className={`text-sm mt-2 ${isCorrect ? "text-green-300" : "text-red-300"}`}
                >
 ...
 * Two-sided templates: 3D flip + “Show back” / “Show front” (matches study layout intent).
 */
export default function CardPreviewContent({
  ...
  const hasQuiz = useMemo(
    () => visibleBlocks.some((b) => isQuizBlock(b)),
    [visibleBlocks],
  );
  ...
  const flipCard = hasFlipBack ? (
    ...
    >
      ...
      >
        ...
        >
          <PreviewBlockList blocks={frontBlocks} {...sharedListProps} />
        </div>
        <div
        ...
        >
          <PreviewBlockList blocks={backBlocks} {...sharedListProps} />
        </div>
      </motion.div>
    </div>
  ) : null;
  ...
  const flipToggle = hasFlipBack ? (
    <button
      ...
      className="flex items-center justify-center gap-2 w-full py-2.5 text-sm font-medium rounded-lg border border-white/15 bg-white/5 hover:bg-white/10 text-white/90 transition-colors"
    >
  ...
  return (
    <div className={`space-y-3 text-[15px] leading-relaxed ${className}`}>
      {hasFlipBack ? (
        ...
        <>
          <PreviewBlockList blocks={blocks || []} {...sharedListProps} />
          {revealSection}
        </>
      )}
    </div>
