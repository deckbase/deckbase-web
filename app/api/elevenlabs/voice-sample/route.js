...
export async function GET(request) {
  ...
  try {
    ...
    if (!fileExists) {
      console.log(
        "voice-sample: Storage miss, calling ElevenLabs once then saving",
        { voiceId, path },
      );
      ...
      if (!response.ok) {
        ...
        console.error(
          "ElevenLabs sample generation failed:",
          response.status,
          errText,
        );
        return NextResponse.json(
      ...
      try {
        ...
        console.error("voice-sample: save failed", {
          bucket: bucket.name,
          path,
          voiceId,
          err: saveErr?.message,
        ...
        return NextResponse.json(
          {
            ...
            hint:
              statusCode === 403 ||
              /permission|403|insufficient/i.test(
                ...
                : "Ensure the Firebase Admin service account has Storage Object Admin (or Storage Admin) on the bucket. Bucket: " +
                  bucket.name,
          },
          { status: 500 },
        );
      ...
      console.log(
        "voice-sample: saved to Storage; future requests will use this file",
        { voiceId, path, bucket: bucket.name },
      );
    } else {
      console.log(
        "voice-sample: Storage hit — serving from Storage, not calling ElevenLabs",
        { voiceId },
      );
    }
    ...
    try {
      ...
      console.error("voice-sample: makePublic failed", {
        voiceId,
        err: makePublicErr?.message,
      });
      return NextResponse.json(
        {
    ...
    const isPermission = /permission|PERMISSION_DENIED|403|insufficient/i.test(
      message,
    );
    return NextResponse.json(
      {
