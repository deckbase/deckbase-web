/**
 * Image block config: crop aspect ratio + default text block for AI image prompt.
 * Stored in block.configJson (template + card blocks_snapshot).
...
/**
 * Default text block whose content pre-fills “Generate with AI” (same keys as audio TTS).
 * @param {string|object|null} configJson
 * @param {{ mainBlockId?: string }} [options]
 * @returns {{ defaultSourceBlockId?: string }}
 */
export function parseImageBlockConfig(configJson, options = {}) {
  const raw =
