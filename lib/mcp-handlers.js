  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          "After the user selects a template, returns exact JSON for that template: each blockId, type, label, configJson, category, plus valuesExample (empty card values) and create_card hints (block_text keys, voice_id / audio_language + audio_gender for TTS). **If any block has type quiz*, use the returned configJson and hints so answers/options match the template.** Requires hosted MCP. Pass templateId from list_templates, or deckId only to use the deck’s default template.",
        inputSchema: {
          type: "object",
          properties: {
            templateId: {
