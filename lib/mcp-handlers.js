  ...
  return {
    tools: [
      ...
      },
      {
        name: "create_cards",
        description:
          `Bulk create cards in one deck using the same template resolution and validation as create_card (deck default or explicit templateId). Requires hosted MCP. Pass cards: array of objects, each with optional front, block_text, generate_audio, voice_id, audio_language, audio_gender (per-card). Optional top-level voice_id, audio_language, audio_gender, and generate_audio apply to the whole batch when not set per card. Max ${MCP_MAX_BULK_CARDS} cards per request. If one card fails, earlier cards may already exist in Firestore.`,
        inputSchema: {
          type: "object",
          properties: {
            deckId: { type: "string" },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            blocks: {
              type: "array",
              description:
                "Optional full block definitions. Mutually exclusive with block_types. Each: blockId (optional), type, label, required (optional), configJson (optional). For audio blocks, include configJson.defaultVoiceId or pass top-level voice_id (or audio_language + audio_gender). Empty or omitted uses header1 Question + hiddenText Answer unless block_types is set.",
              items: { type: "object" },
            },
            voice_id: {
              type: "string",
      ...
      },
      {
        ...
        description:
          "Update an existing template’s name, description, block layout (blocks or block_types), rendering, main/sub block ids, and/or default voice for audio blocks. Requires hosted MCP. Omit blocks and block_types to keep layout; version increments when anything changes. If the layout includes audio without defaultVoiceId, ask the user for voice settings first, then pass voice_id or audio_language + audio_gender (same as create_template).",
        inputSchema: {
          type: "object",
          properties: {
