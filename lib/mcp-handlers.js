  ...
  return {
    tools: [
      ...
      },
      {
        name: "create_cards",
        description:
          `Bulk create cards in one deck using the same template resolution and validation as create_card (deck default or explicit templateId). Requires hosted MCP. Pass cards: array of objects, each with optional front and block_text (same shape as create_card). Max ${MCP_MAX_BULK_CARDS} cards per request. If one card fails, earlier cards may already exist in Firestore.`,
        inputSchema: {
          type: "object",
          properties: {
            deckId: { type: "string" },
            ...
            cards: {
              type: "array",
              description: `Non-empty array of per-card payloads (front?, block_text?). Max ${MCP_MAX_BULK_CARDS} items.`,
            },
          },
      ...
      },
      {
        ...
        description:
          "Create a flashcard template (block layout for new cards). Requires hosted MCP with API key. Call list_template_block_types first so the user can pick block types; pass the ordered selection as block_types. Alternatively pass full blocks, or omit both for default Question + Answer.",
        inputSchema: {
          type: "object",
          properties: {
            ...
            blocks: {
              type: "array",
              description:
                "Optional full block definitions. Mutually exclusive with block_types. Each: blockId (optional), type, label, required (optional), configJson (optional). Empty or omitted uses header1 Question + hiddenText Answer unless block_types is set.",
              items: { type: "object" },
            },
            rendering: {
              type: "object",
