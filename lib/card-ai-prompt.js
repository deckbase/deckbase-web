...
function blockListLine(block) {
  ...
  const t = block.type;
  if (t === "audio" || t === 7 || t === "7") {
    return `- ${block.blockId} (${label}): string — text to speak for this card`;
  }
  if (t === "quizSingleSelect" || t === 9 || t === "9") {
    return `- ${block.blockId} (${label}): object — {"question":"...","options":["≥2 strings"],"correctIndex":0} (0-based index of the one correct option). Alternatively use "correctAnswer":"exact option text" instead of correctIndex.`;
  }
  if (t === "quizMultiSelect" || t === 8 || t === "8") {
    return `- ${block.blockId} (${label}): object — {"question":"...","options":["≥2 strings"],"correctIndices":[0,2]} (0-based indices of all correct options). Alternatively "correctAnswers":["opt text",...] matching option strings exactly.`;
  }
  if (t === "quizTextAnswer" || t === 10 || t === "10") {
    return `- ${block.blockId} (${label}): object — {"question":"...","correctAnswer":"..."} (correctAnswer = text the learner must type; optional "hint":"..." ).`;
  }
  return `- ${block.blockId} (${label}): string — block content`;
}
...
export function buildCardPrompt({
  ...
  const blockList = blocksForList.map(blockListLine).join("\n");
  const hasQuiz = blocksForList.some(isQuizBlock);
  ...
  const mainSubHint =
    (mainBlockId || subBlockId) && numCards > 1
      ? `\nThe template's main/front field is ${mainBlockId || "—"}${mainBlockLabel ? ` (${mainBlockLabel})` : ""}${mainIsQuiz ? " (quiz — use a different question each card)" : ""}; the sub/back field is ${subBlockId || "—"}${subBlockLabel ? ` (${subBlockLabel})` : ""}${subIsQuiz ? " (quiz — vary questions/answers)" : ""}. Each card must be clearly different — do not repeat the same main question, main text, or sub content.`
      : "";

  const mainSubHintSingle =
    mainBlockId || subBlockId
      ? ` Main/front field: ${mainBlockId || "—"}${mainBlockLabel ? ` (${mainBlockLabel})` : ""}${mainIsQuiz ? " (quiz object)" : ""}. Sub/back field: ${subBlockId || "—"}${subBlockLabel ? ` (${subBlockLabel})` : ""}${subIsQuiz ? " (quiz object)" : ""}.`
      : "";
  ...
  const jsonExampleOne =
    blocksForList.length > 0
      ? `{${blocksForList.map(jsonFragmentForBlock).join(", ")}}`
      : "{}";
  ...
  const quizSec = hasQuiz ? `\n5) ${buildQuizPromptBody(blocksForList)}\n` : "";
...
/**
 * Build prompt for File-to-AI-Cards. Used when the user has mapped columns to text
 * blocks and enabled "Use AI" for quiz/audio blocks. AI generates only quiz and
 * audio content per row; text blocks are filled by the user's column mapping.
 *
 * @param {Object} opts
 * @param {string} opts.extractedContent
 * @param {{ blockId: string, type: string|number, label: string }[]} opts.templateBlocks
...
export function buildCardPromptFromContent({
  ...
  const blockList = blocks.map(blockListLine).join("\n");
  const hasQuiz = blocks.some(isQuizBlock);
  const hasAudio = blocks.some((b) => b.type === "audio" || b.type === 7 || b.type === "7");
  const jsonExampleOne =
    blocks.length > 0 ? `{${blocks.map(jsonFragmentForBlock).join(", ")}}` : "{}";
  const jsonExample =
    `[${Array.from({ length: Math.min(2, numCards) }, () => jsonExampleOne).join(", ")}]`;

  let valueRules;
  if (hasQuiz) {
    valueRules = `Generate content only for quiz and audio blocks (nested objects for quiz — see section 4). Use \"\" for text blocks; the user maps those from columns.${QUIZ_RULES}`;
  } else if (hasAudio) {
    valueRules = "Generate content only for audio blocks (spoken text as string). Use \"\" for text blocks (user fills via column mapping).";
  } else {
    valueRules = "Use \"\" for all blocks (user fills text via column mapping; Use AI is for quiz/audio only).";
  }

  const system = `You are a flashcard generator. You receive (1) a template with blockIds and labels, and (2) extracted table content. The user has already mapped table columns to text blocks. Your task is to generate only quiz and audio block content for each row (one card per row). Output up to ${numCards} cards as a JSON array of objects. ${valueRules} Return only JSON. No markdown, no code fence. Use exact blockIds (UUIDs). Each object must include every blockId: generated content for quiz/audio, \"\" for text blocks.`;

  const quizSec = hasQuiz ? `\n4) ${buildQuizPromptBody(blocks)}\n` : "";
  const lastStep = hasQuiz ? "5)" : "4)";

  const user = `1) Template:
${blockList}

2) Deck context: ${deckTitle || "(untitled)"}

3) Extracted table (user maps columns to text blocks; you generate only quiz/audio per row):
---
${(extractedContent || "").trim().slice(0, 45000)}
---
${quizSec}
${lastStep} Generate up to ${numCards} cards. One card per row. Return a JSON array. For text blocks use \"\"; generate only quiz and audio blocks. Example shape: ${jsonExample}`;
...
export function buildImportQuizAudioPrompt({
  ...
  const hasQuiz = blocks.some(isQuizBlock);
  const hasAudio = blocks.some((b) => b.type === "audio" || b.type === 7 || b.type === "7");
  const taskDesc = importAiTaskDescription(blocks);
  const jsonExampleOne =
    `{${blocks.map(jsonFragmentForBlock).join(", ")}}`;
  const jsonExample =
    `[${Array.from({ length: Math.min(2, numCards) }, () => jsonExampleOne).join(", ")}]`;
  ...
  const user = `1) Blocks to generate (${taskDesc}):
${blockList}

2) Deck context: ${deckTitle || "(untitled)"}

3) Extracted table (one card per row; generate only the blocks above):
---
${(extractedContent || "").trim().slice(0, 45000)}
---
${quizSec}
...
export function buildCardPromptFromImage({
  ...
  if (hasQuiz) {
    valueRules = `String values for text/audio; nested objects for quiz blocks (see section 4).${QUIZ_RULES}`;
  } else if (hasAudio) {
    valueRules = "Each key = blockId, value = string. For audio blocks provide spoken text as a string.";
  }

  const system = `You are a flashcard generator. You receive (1) a template and (2) an image. Read all visible text, diagrams, and key concepts in the image. MAP them to template fields. Output up to ${numCards} cards as a JSON array of objects. ${valueRules} Return only JSON. No markdown, no code fence. Use exact blockIds (UUIDs).`;
...
3) Analyze the attached image and generate up to ${numCards} flashcards from its content.
${quizSec}
${lastStep} Return a JSON array. Example shape: ${jsonExample}`;
