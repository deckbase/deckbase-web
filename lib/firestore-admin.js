...
import { generateFalTextToImageServer } from "@/lib/ai-image-generation";
...
/** Upload image buffer to Storage and create media doc. Returns { mediaId }. */
export async function uploadImageBufferAdmin(uid, buffer, mimeType = "image/jpeg") {
  const db = getDb();
  const bucket = getAdminBucket();
  if (!db || !bucket) throw new Error("Firebase Admin Storage not configured");
  const mediaId = uuidv4();
...
/**
 * fal.ai T2I for an existing card’s image block; appends or replaces mediaIds on success.
 * @param {object} toolOptions - prompt (required), model_id?, style_prompt_id?, block_id?, replace_existing?
 */
export async function attachImageToExistingCardAdmin(uid, deckId, cardId, toolOptions = {}) {
  ...
  const prompt = String(toolOptions.prompt ?? "").trim();
  if (!prompt) throw new Error("prompt is required");

  const modelId =
    String(toolOptions.model_id ?? toolOptions.modelId ?? "fal-ai/flux/schnell").trim() ||
    "fal-ai/flux/schnell";
  const stylePromptId = String(
    toolOptions.style_prompt_id ?? toolOptions.stylePromptId ?? "",
  ).trim();

  if (process.env.NODE_ENV === "production") {
    const entitled = await isBasicOrProOrVip(uid);
    if (!entitled) {
      throw new Error("Active subscription required for AI image generation");
    }
  }

  const limitCheck = await checkImageGenerationLimit(uid, modelId);
  if (!limitCheck.allowed) {
    throw new Error(limitCheck.message || "Monthly AI image credit limit reached");
  }

  const card = await getCardAdmin(uid, cid);
  if (!card) throw new Error("Card not found");
  if (card.deckId !== did) throw new Error("Card does not belong to this deck");

  const blocksSnapshot = card.blocksSnapshot || [];
  const imageBlocks = blocksSnapshot.filter((b) => isImageBlockType(b));
  if (imageBlocks.length === 0) {
    throw new Error("Card has no image block in blocksSnapshot");
  }

  const blockIdHint =
    toolOptions.block_id ?? toolOptions.blockId ?? toolOptions.image_block_id;
  let targetBlock;
  if (blockIdHint) {
    targetBlock = imageBlocks.find((b) => b.blockId === blockIdHint);
    if (!targetBlock) {
      throw new Error(
        `No image block with block_id "${blockIdHint}". Image blockIds: ${imageBlocks.map((b) => b.blockId).join(", ")}`,
      );
    }
  } else if (imageBlocks.length > 1) {
    throw new Error(
      `Card has multiple image blocks; pass block_id. Available: ${imageBlocks.map((b) => b.blockId).join(", ")}`,
    );
  } else {
    targetBlock = imageBlocks[0];
  }

  const values = JSON.parse(JSON.stringify(card.values || []));
  const imageIdx = values.findIndex((v) => v.blockId === targetBlock.blockId);
  if (imageIdx < 0) {
    throw new Error("Card values are missing an entry for this image block");
  }

  const replaceExisting =
    toolOptions.replace_existing === true || toolOptions.replaceExisting === true;
  const currentMediaIds = values[imageIdx].mediaIds || [];

  const { imageUrl, modelId: resolvedModelId } = await generateFalTextToImageServer({
    modelId,
    prompt,
  ...
  const { mediaId } = await uploadImageBufferAdmin(uid, buffer, mime);
  await incrementImageUsage(uid, resolvedModelId);

  if (replaceExisting) {
    values[imageIdx].mediaIds = [mediaId];
  } else {
    values[imageIdx].mediaIds = [...currentMediaIds, mediaId];
  }

  await updateCardValuesAdmin(uid, cid, did, values);
