...
export async function getTemplateAdmin(uid, templateId) {
  const db = getDb();
  if (!db) return null;
  const ref = db.collection("users").doc(uid).collection("templates").doc(templateId);
  const snap = await ref.get();
  if (!snap.exists) return null;
  const d = snap.data();
  return {
...
export async function getCardsForExamplesAdmin(uid, deckId, limitCount = 5) {
  ...
  snap.docs.forEach((docSnap) => {
    ...
    if ((!values || values.length === 0) && d.values_json) {
      try {
        const parsed = typeof d.values_json === "string" ? JSON.parse(d.values_json) : d.values_json;
...
export async function createCardAdmin(uid, deckId, templateId, blocksSnapshot, values, mainBlockId = null, subBlockId = null) {
  const db = getDb();
  ...
  const deckRef = db.collection("users").doc(uid).collection("decks").doc(deckId);
