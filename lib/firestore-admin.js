...
export async function getTemplateAdmin(uid, templateId) {
  ...
  const rend = d.rendering;
  const rendering =
    rend && typeof rend === "object"
      ? {
          frontBlockIds: rend.front_block_ids ?? rend.frontBlockIds ?? [],
          backBlockIds: rend.back_block_ids ?? rend.backBlockIds ?? [],
        }
      : null;
 ...
 * @param {string} [options.voice_id] - ElevenLabs voice id when generating audio. Required if generate_audio and template audio block(s) lack defaultVoiceId; use list_elevenlabs_voices after user picks. Overrides template defaults when set.
 */
export async function createCardFromTemplateAdmin(
  ...
  const voiceIdFromOpts = String(options.voice_id ?? options.voiceId ?? "").trim();
  if (
    genAudio &&
    templateNeedsExplicitVoiceForMcpTts(blocksSnapshot, template.mainBlockId) &&
    !voiceIdFromOpts
  ) {
    throw new Error(
      'voice_id is required for audio generation when the template audio block(s) have no defaultVoiceId. Call list_elevenlabs_voices, ask the user which voice to use, then pass voice_id. Or set generate_audio: false, or set defaultVoiceId on the template audio block in the dashboard.',
    );
  }
  await applyElevenLabsTtsForMcpCard(
    ...
    uploadAudioBufferAdmin,
    {
      ...
      voiceIdOverride: voiceIdFromOpts || undefined,
    },
  );
...
export async function buildMcpTemplateCardSchema(uid, templateId) {
  ...
  return {
    ...
    create_card: {
      ...
      voice_id:
        "When generate_audio is true: required if voice_id_required_for_tts is true (no defaultVoiceId on all audio blocks) — call list_elevenlabs_voices, ask the user which voice, then pass id. Optional if template audio block(s) already define defaultVoiceId; when set, overrides template defaults for TTS.",
      note: "Pass this templateId to create_card (or omit if deck default is this template). Image blocks still need uploads in-app. Audio: when ELEVENLABS_API_KEY is set, create_card runs ElevenLabs TTS from non-empty text (audio block text, or defaultSourceBlockId / main / first text block) and sets mediaIds; use generate_audio: false to skip. Same TTS/storage limits as the web API.",
    },
  };
}
...
export async function createTemplateAdmin(uid, opts = {}) {
  ...
  const voiceIdOpt = String(opts.voice_id ?? opts.voiceId ?? "").trim();
  if (voiceIdOpt) {
    blocks = blocks.map((b) => {
  ...
  if (templateNeedsExplicitVoiceForMcpTts(blocks, mainBlockId)) {
    throw new Error(
      'voice_id is required when the template includes an audio block without defaultVoiceId. Call list_elevenlabs_voices, ask the user which voice to use, then pass voice_id, or set defaultVoiceId in each audio block\'s configJson in blocks.',
    );
  }
 ...
 * Omit blocks and block_types to keep the current layout; pass voice_id to set defaultVoiceId on audio blocks.
 */
export async function updateTemplateAdmin(uid, templateId, opts = {}) {
  ...
  const renderingIn = opts.rendering;
  let rendering;
  if (renderingIn !== undefined) {
    rendering =
      renderingIn && typeof renderingIn === "object"
        ? {
            front_block_ids:
              renderingIn.frontBlockIds ?? renderingIn.front_block_ids ?? [],
            back_block_ids:
              renderingIn.backBlockIds ?? renderingIn.back_block_ids ?? [],
          }
        : null;
  } else {
    const er = existing.rendering;
    rendering =
      er && (er.frontBlockIds?.length || er.backBlockIds?.length)
        ? {
            front_block_ids: er.frontBlockIds ?? [],
            back_block_ids: er.backBlockIds ?? [],
          }
        : null;
  }
 ...
 * @param {object} toolOptions - voice_id (required), block_id?, text?, replace_existing?, generate_audio?
 */
export async function attachAudioToExistingCardAdmin(uid, deckId, cardId, toolOptions = {}) {
  const voiceIdRequired = String(toolOptions.voice_id ?? toolOptions.voiceId ?? "").trim();
  if (!voiceIdRequired) {
    throw new Error(
      'voice_id is required. Call list_elevenlabs_voices, ask the user which voice to use, then pass voice_id.',
    );
  }
