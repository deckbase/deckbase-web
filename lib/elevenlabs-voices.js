...
const CURATED_LANGS = [
  ...
  ["tr", "Turkish", "FDs1ZX5J4e4f2c2erxtW", "bKzPIaBn2Q715Nv8nEwW"],
...
export async function getElevenlabsVoicesMcpPayload(toolArgs = {}) {
  ...
  return {
    ...
    hint:
      "Use each voice's id as voice_id on create_card / create_template / update_template / attach_audio_to_card, or pass audio_language + audio_gender instead. " +
      "list_elevenlabs_voices optional args: language, gender, search. " +
      "For templates with audio blocks, set default voice via voice_id or audio_language+audio_gender on create_template (stored as defaultVoiceId).",
