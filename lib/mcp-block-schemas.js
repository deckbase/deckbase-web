...
export function getDeckbaseBlockSchemas() {
  return {
    ...
    createCardMcpNote:
      "create_card / create_cards accept block_text: { [blockId]: string } for text fields. Image blocks still need uploads in-app. Audio: with ELEVENLABS_API_KEY, TTS runs when generate_audio is true and source text exists. Ask the user for voice settings, then pass voice_id from list_elevenlabs_voices OR audio_language (ISO 639) and audio_gender (female|male). attach_audio_to_card accepts the same. Production enforces TTS/storage limits like the web API.",
    docsPaths: [
