...
export function getDeckbaseBlockSchemas() {
  return {
    ...
    createCardMcpNote:
      "create_card / create_cards accept block_text: { [blockId]: string } for text fields. Image blocks still need uploads in-app. Audio: with ELEVENLABS_API_KEY, TTS runs when generate_audio is true and source text exists. Pass voice_id from list_elevenlabs_voices after the user picks a voice when get_template_schema.voice_id_required_for_tts is true (no defaultVoiceId on all audio blocks); optional if template already sets defaultVoiceId. attach_audio_to_card always requires voice_id. Production enforces TTS/storage limits like the web API.",
    docsPaths: [
