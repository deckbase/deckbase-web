...
export function validateClientRedirect(clientId, redirectUri) {
  ...
  if (clientId === "deckbase" && isAllowedChatgptOpenAiBrowserRedirect(redirectUri)) {
    return { ok: true, client };
  }
  const allowed = client.redirect_uris || [];
  const exact = allowed.some((u) => u === redirectUri);
  if (exact) return { ok: true, client };
  const prefix = allowed.some((u) => redirectUri.startsWith(u + "/") || redirectUri.startsWith(u + "?"));
